...
const PhotoCaptureForm = ({ setOpen }) => {
  ...
  const [imageCaptured, setImageCaptured] = useState(false);
  const [profileImageBlob, setProfileImageBlob] = useState(null);
  const [capturedBlob, setCapturedBlob] = useState(null); // State to store captured image blob
  const [isUploading, setIsUploading] = useState(false);
  const { authToken } = useGetUser();
  ...
  const takePicture = async () => {
    try {
      ...
      const dataUrl = photo.toDataURL("image/png");
      console.log("Data URL generated:", dataUrl); // Debugging

      // Create Blob from data URL
      const response = await fetch(dataUrl);
      const imgBlob = await response.blob();

      console.log("Captured Image Blob:", imgBlob); // Ensure Blob is created
      setCapturedBlob(imgBlob); // Save captured Blob in state
  ...
  const compareFaces = async (capturedImage, profileImage) => {
    try {
      if (!(capturedImage instanceof Blob)) {
        console.error("Captured image is not a Blob:", capturedImage);
        return;
      }

      if (!(profileImage instanceof Blob)) {
        console.error("Profile image is not a Blob:", profileImage);
        return;
      }
  ...
  const clearImage = () => {
    ...
    ctx.clearRect(0, 0, photo.width, photo.height);
    setImageCaptured(false);
    setCapturedBlob(null); // Reset captured blob
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex w-full justify-between">
        ...
        <Button
          onClick={() => compareFaces(capturedBlob, profileImageBlob)}
          variant="contained"
          disabled={
            !imageCaptured ||
            isUploading ||
