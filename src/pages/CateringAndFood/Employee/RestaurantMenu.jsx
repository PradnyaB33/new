...
function RestaurantMenu() {
  ...
  const fetchList = useCallback(async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_API}/route/menu/items/${_id}`, {
        headers: {
          "Content-Type": "application/json",
          Authorization: authToken,
        },
  ...
  const fetchLikedItems = useCallback(async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_API}/route/likedItem`, {
        params: {
          userId,
          vendorId: _id,
        },
  ...
  const fetchFrequentlyOrderedItems = useCallback(async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_API}/route/frequentlyordered/${userId}/${_id}`, {
        headers: {
          "Content-Type": "application/json",
          Authorization: authToken,
        },
  ...
  const toggleFavorite = async (itemName, isFavorite) => {
    try {
      if (isFavorite) {
        const response = await axios.delete(`${process.env.REACT_APP_API}/route/likedItem/unlike`, {
          data: { userId, itemName, vendorId: _id },
          headers: {
            "Content-Type": "application/json",
            Authorization: authToken,
        ...
        const response = await axios.post(`${process.env.REACT_APP_API}/route/likedItem/like`, {
          userId,
          itemName,
          vendorId: _id,
        }, {
          ...
          <div className="mt-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
              {likedItems.length > 0 ? (
                  ...
                  return item ? (
                    <div key={item._id} className="bg-white rounded-lg shadow-lg p-4 flex flex-col transition-transform transform hover:scale-105 relative">
                      ...
                      <div className="flex justify-around pb-3 items-center mb-2">
                        <button onClick={() => toggleFavorite(item.name, true)} className="text-red-600">❤️</button>
                        <div className="flex items-center">
                          <button onClick={() => decrement(item._id)} className="bg-red-600 text-white rounded-l-lg px-3">-</button>
                          <span className="px-4">{cart[item._id]?.count || 0}</span>
                          <button onClick={() => increment(item._id)} className="bg-blue-600 text-white rounded-r-lg px-3">+</button>
                        </div>
                      </div>
                      <button onClick={() => addToCart(item)} className="mt-auto bg-blue-600 text-white rounded-lg py-2 hover:bg-blue-700 transition duration-200">
                        Add to Cart
                      </button>
                    </div>
                  ) : null;
                ...
                <div className="col-span-full text-center text-gray-500">No liked items available.</div>
              )}
            </div>
          </div>
        ) : (
            ...
            {filteredMenu.length > 0 ? (
              filteredMenu.map(item => (
                <div key={item._id} className="bg-white rounded-lg shadow-lg p-4 flex flex-col transition-transform transform hover:scale-105 relative">
                  ...
                  <p className="text-gray-600 mb-2">Preparation Time: {item.preparationTime} min</p>
                  <div className="flex justify-around pb-3 items-center mb-2">
                    <button onClick={() => toggleFavorite(item.name, favorites[item.name])} className="text-red-600">{favorites[item.name] ? '❤️' : '🤍'}</button>
                    <div className="flex items-center">
                      <button onClick={() => decrement(item._id)} className="bg-red-600 text-white rounded-l-lg px-3">-</button>
              ...
              <div className="col-span-1 text-center py-4 text-gray-500">{errorMessage || "No menu items available."}</div>
            )}
          </div>
          </div>
                  </>
