...
const EmpInfoPunchStatus = () => {
  ...
  const [confirmOpenDialogueBox, setConfirmOpenDialogueBox] = useState(null);
  const handleOpenBox = (employee) => {
    setConfirmOpenDialogueBox(employee);
  };

  const handleCloseBox = () => {
    setConfirmOpenDialogueBox(null);
  };
  return (
    <>
      <Container maxWidth="xl" className="bg-gray-50 min-h-screen">
        <article className="SetupSection bg-white w-full h-max shadow-md rounded-sm border items-center">
          ...
          <p className="text-xs text-gray-600 pl-10 text-center">
            Track the attendance of employee here by using sync button.
          </p>
                ...
                {availableEmployee
                  .filter((item) => {
                    return (
                      ...
                      (!empIdSearch.toLowerCase() ||
                        (item.empId !== null &&
                          item.empId !== undefined &&
                          item.empId.toLowerCase().includes(empIdSearch)))
                    );
                  })
                  .map((item, id) => (
      ...
      <Dialog open={confirmOpenDialogueBox !== null} onClose={handleCloseBox}>
        <DialogTitle>Confirmation</DialogTitle>
        <DialogContent>
          <p>Do you want to synch it with attendance management?</p>
        </DialogContent>
        <DialogActions>
