  ...
  const handleApprovalUpdate = async () => {
    try {
      const response = await axios.put(
        `${process.env.REACT_APP_API}/route/organization/${organisationId}/update-approvalId`,
        {},
        {
          headers: {
            Authorization: authToken,
                            ...
                            <tbody>
                                {record.unavailableRecords.map((unavailableRecord, id) => (
                                    <tr className="!font-medium border-b" key={id}>
                                        ...
                                        <td className="!text-left pl-4 py-3">{unavailableRecord?.justify || ""}</td>
                                        <td className="!text-left pl-4 py-3">
                                            {role === "Manager" ? (
                                                ...
                                                <>
                                                    <button onClick={() => handleApprovalUnavailableRecord(unavailableRecord._id)} className="bg-green-500 text-white px-2 py-1 rounded-md mr-2">Accept</button>
                                                    <button onClick={() => handleRejectUnavailableRecord(unavailableRecord._id)} className="bg-red-500 text-white px-2 py-1 rounded-md">Reject</button>
                                                    <button onClick={() => handleApprovedLeaveUnavailableRecord(unavailableRecord._id)} className="bg-blue-500 text-white px-2 py-1 ml-2 rounded-md">Approved as leave</button>
                                                </>
                                            ) : null}
                                        </td>
                                    </tr>
